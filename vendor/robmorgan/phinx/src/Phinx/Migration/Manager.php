...
class Manager
{
     ...
     * @return array array indicating if there are any missing or down migration
     */
    public function printStatus(string $environment, ?string $format = null): array
    {
        ...
        if (count($migrations)) {
            ...
            // get the migration sorted in the same way as the versions
            ...
            if (empty($sortedMigrations) && !empty($missingVersions)) {
                // this means we have no up migration, so we write all the missing versions already so they show up
            ...
            // any migration left in the migration (ie. not unset when sorting the migration by the version order) is
            // a migration that is down, so we add them to the end of the sorted migration list
            if (!empty($migrations)) {
            ...
            // and finally add any possibly-remaining missing migration
            foreach ($missingVersions as $missingVersionCreationTime => $missingVersion) {
                $this->printMissingVersion($missingVersion, $maxNameLength);
            ...
            $output->writeln('', $this->verbosityLevel);
            $output->writeln('There are no available migration. Try creating one using the <info>create</info> command.', $this->verbosityLevel);
        }
    ...
    {
        // note that the migration are indexed by name (aka creation time) in ascending order
        $migrations = $this->getMigrations($environment);
        ...
        // get a list of migration sorted in the opposite way of the executed versions
        $sortedMigrations = [];
        ...
        foreach ($executedVersions as $versionCreationTime => &$executedVersion) {
            ...
            if (isset($migrations[$versionCreationTime])) {
                ...
                // migration (or choosing the last up version as target)
        ...
        if ($target === 'all' || $target === '0') {
            ...
            // search through the migration using the name
        ...
        if (empty($executedVersionCreationTimes) || $target == end($executedVersionCreationTimes)) {
            $this->getOutput()->writeln('<error>No migration to rollback</error>');
    ...
    /**
     * Gets an array of the database migration, indexed by migration name (aka creation time) and sorted in ascending
     * order
     *
     * @param string $environment Environment
     * @throws \InvalidArgumentException
