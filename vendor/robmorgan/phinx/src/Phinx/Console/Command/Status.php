...
class Status extends AbstractCommand
{
    ...
    protected function configure(): void
    {
        ...
        $this->setDescription('Show migration status')
            ...
            ->setHelp(
                <<<EOT
The <info>status</info> command prints a list of all migration, along with their current status

<info>phinx status -e development</info>
<info>phinx status -e development -f json</info>

The <info>version_order</info> configuration option is used to determine the order of the status migration.
EOT
            );
    }
     ...
     * @return int 0 if all migration are up, or an error code
     */
