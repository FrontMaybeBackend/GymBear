...
class Migrate extends AbstractCommand
{
    ...
    protected function configure(): void
    {
        ...
        $this->setDescription('Migrate the database')
            ...
            ->addOption('--fake', null, InputOption::VALUE_NONE, "Mark any migration selected as run, but don't actually execute them")
            ->setHelp(
                <<<EOT
The <info>migrate</info> command runs all available migration, optionally up to a specific version
...
EOT
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if ($fake) {
            $output->writeln('<comment>warning</comment> performing fake migration', $this->verbosityLevel);
        }
