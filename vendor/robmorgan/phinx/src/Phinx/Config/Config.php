...
class Config implements ConfigInterface, NamespaceAwareInterface
{
    ...
    {
        if (!isset($this->values['paths']['migration'])) {
            throw new UnexpectedValueException('Migrations path missing from config file');
        }

        if (is_string($this->values['paths']['migration'])) {
            $this->values['paths']['migration'] = [$this->values['paths']['migration']];
        }

        return $this->values['paths']['migration'];
    }
